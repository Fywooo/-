...
# 单个Encoder层，就是一个attention和一个MLP
class EncoderLayer(nn.Module):

    def __init__(self, size: int, self_attn, MLP, dropout: float = 0.1, rezero=True):
        super(EncoderLayer, self).__init__()
        self.self_attn = self_attn
        self.MLP = MLP
        self.sublayer = clones(ResidualAdd(size, dropout, rezero), 2) # 用到了两个残差链接
        self.size = size
...
class VIT_hybrid(nn.Module):
    def __init__(self, args, in_channels: int = 3, patches: int = 16, emb_size: int = 768, num_head: int = 8,
        ...
        self.encodelayer = EncoderLayer(emb_size, MultiHeadAttention(emb_size, num_head, self.dropout, seq_len),
                                        MLP(emb_size, 4, dropout=self.dropout), dropout=self.dropout,
                                        rezero=args.re_zero)
...
class VIT_hybrid_1(nn.Module):
    def __init__(self, args, in_channels: int = 3, patches: int = 16, emb_size: int = 768, num_head: int = 8,
                 img_size: int = 224, depth: int = 12, n_classes: int = 2):
        super(VIT_hybrid_1, self).__init__()
        if args.in_channels:
            in_channels = args.in_channels
        seq_len = img_size//patches ** 2 + 1
        self.depth = depth
        ...
        self.embed = PatchEmbedding(args.resnet_out_channels, 1, emb_size, seq_len)
        self.CNNnet = CNN(args, in_channels, args.resnet_out_channels)
