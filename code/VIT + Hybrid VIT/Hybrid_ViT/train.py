...
from models.VIT import VIT_hybrid_1
from models.VIT import VIT_hybrid



from utils.ResidualNet import Res34, CNN
from data.dataload import VITSet
        ...
        with tqdm(total=len(train_loader)) as t:
            ...
            print(
                '\nEpoch:{:d},loss:{:.4f},Precission:{:.4f},Recall:{:.4f},F1:{:.4f}'.format(epoch, val_loss, P,
                                                                                            R, F1))
            if epoch % 2 == 0 or epoch >= 43:
                torch.save(model.state_dict(), "./checkpoint/vit_hybrid_model_epoch{:d}.pth".format(epoch))
            file_save_name = "./checkpoint/vit_hybrid_model_{:d}".format(epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--batch_size', type=int, default=1, help='input batch size for training (default: 32)')
